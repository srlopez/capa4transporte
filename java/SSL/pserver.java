...
public class pserver implements Runnable {
    ...
    private String[] map = {
            ...
            "-  Buenos días. Busco trabajo.@@" + "- ¿Le interesa de jardinero?@@"
                    + "- ¿Dejar dinero? ¡Si lo que busco es trabajo!",
            "- ¿Qué es un pez en un cine?@@" + "- Un mero espectador",
            "- Hola, soy paraguayo y quiero pedirle la mano de su hija para casarme con ella.@@" + "- ¿Para qué?@@"
                    + "- Paraguayo.",
            "- ¿Para que van una caja al gimnasio?@@" + "-  Para hacerse caja fuerte.",
            ...
            "- Hola, me llamo Paco. ¿Y tú?@@" + "- No, yo no",
            "- Intenté enamorar a una programadora, pero no se de Java@@" + "- ¿Has probado com Pilar?",
            "¿QuiénSabeComoArreglarLaBarraEspaciadora?.", "- ¿Qué son 8 bocabits?@@" + "- Un bocabyte.",
            "- Dime con quién andas y te diré quién eres.@@" + "- No ando con nadie...@@" + "- Eres feo.",
            "- ¡Rápido, necesitamos sangre!@@" + "- Yo soy 0 positivo.@@"
            ...
            "- Te detesto.@@" + "- K de kilo.@@" + "- ¿Qué dices?@@" + "- No sé... has empezado tú.",
            "- No tienes ni idea de lo que soporto@@" + "- Una ciudad de Portugal",
            "- Papá, que significa sintaxis@@" + "- Que tienes que coger el autobús@@" + "- Gracias papá, eres genial@@"
                    + "- Uno que ha estudiao",
            "- Una cita por Tinder, todo son nervios:@@" + "- Hola, soy Antonio@@"
            ...
            "- ¡Hombre! ¡Cristiano Ronaldo! Beienvenido@@"
                    ...
                    + "- Será a la romana.@@" + "- Irina, cariño, dile al camarero este de dónde eres...",
            "- Tu rezas antes de comer.@@" + "- No señor. Mi madre es buena cocinera.",
            ...
            "-  Amor mío, me gustaría que todo volviese a ser como antes.@@" + "- ¿Cómo cuando nos conocimos?@@"
                    + "- No, antes...",
            "- Dos cojos (con pata de palo),... y se va la luz en el ascensor:@@" + "- ¡Qué mala pata!@@"
                    + "- ¡Quema la tuya!!!" };

    public pserver(Socket client) {
        connect = client;
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            while (true) {
                pserver miServer = new pserver(socket.accept());
    ...
    public void run() {
        ...
        try {
            ...
            do {
                // LEEMOS CABECERA
                input = in.readLine(); // <-- LINEA DE CABECERA DEL MENSAJE
                ...
                case "MSG":
                    System.out.println("MSG recibido: " + input + " (" + ip + ")");
                    // LO MOSTRAMOS
                    i = 0;
                    for (String s : body)
                ...
                default: // ERROR
                    System.out.println("ERROR NO IDENTIFICADO: " + input + " (" + ip + ")");
                }

            } while (params[0].compareTo("END") != 0);
